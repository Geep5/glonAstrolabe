...
1. **All fresh installs join the same default auction-house network.**  
   The autobase bootstrap key is hardcoded in the source (`src/autobase-host.ts`). Fresh installs automatically join this network without any env vars. If you want a private auction house, set `GLON_AUTOBASE_BOOTSTRAP=<your-64-char-hex-key>` before first start, or wipe `~/.glon/autobase/bootstrap.key` and restart.

2. **Daemon needs `GLON_AUCTION=1`.**  
...
3. **Wallet keys live in the daemon, not the actor host.**  
